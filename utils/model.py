...
class MMIUNet_GuideDecoder(nn.Module):
    ...
    def __init__(self, bert_type, vision_type, project_dim=512, model_save_filename='mmiunet_gd'):
        ...
        use_fuzzy = False
        use_pmm = False
        if 'fuzzy' in model_save_filename:
            use_fuzzy = True
        if 'pmm' in model_save_filename:
            use_pmm = True
        
        self.fusion1 = Bridger(d_img=channels[0], d_model=channels[0], stage_id=1, use_fuzzy=use_fuzzy)
        self.fusion2 = Bridger(d_img=channels[1], d_model=channels[1], stage_id=2, use_fuzzy=use_fuzzy)
        self.fusion3 = Bridger(d_img=channels[2], d_model=channels[2], stage_id=3, use_fuzzy=use_fuzzy)
        self.fusion4 = Bridger(d_img=channels[3], d_model=channels[3], stage_id=4, use_fuzzy=use_fuzzy)

        self.decoder16 = GuideDecoder(feature_dim[0],feature_dim[1],self.spatial_dim[0],24,use_pmm=use_pmm)
        self.decoder8 = GuideDecoder(feature_dim[1],feature_dim[2],self.spatial_dim[1],12,use_pmm=use_pmm)
        self.decoder4 = GuideDecoder(feature_dim[2],feature_dim[3],self.spatial_dim[2],9,use_pmm=use_pmm)
        self.decoder1 = SubpixelUpsample(2,feature_dim[3],24,4)
        self.out = UnetOutBlock(2, in_channels=24, out_channels=1)

    def forward(self, data):
        encoder_feats, skip_connection = [], []
        image, text = data
        if image.shape[1] == 1:   
            image = repeat(image,'b 1 h w -> b c h w',c=3)
        ...
        os16 = self.decoder16(os32, encoder_feats[2], text_embeds[-1], skip_connection[2])
        os8 = self.decoder8(os16, encoder_feats[1], text_embeds[-1], skip_connection[1])
        os4 = self.decoder4(os8, encoder_feats[0], text_embeds[-1], skip_connection[0])
        os4 = rearrange(os4, 'B (H W) C -> B C H W',H=self.spatial_dim[-1],W=self.spatial_dim[-1])
        os1 = self.decoder1(os4)
...
class MMIUNet_V2(nn.Module):
    ...
    def __init__(self, bert_type, vision_type, project_dim=512, model_save_filename='mmiunet'):
        ...
        self.decode4 = Decoder(channels[3],channels[2],use_pmm=use_pmm)
        self.decode3 = Decoder(channels[2],channels[1],use_pmm=use_pmm)
        self.decode2 = Decoder(channels[1],channels[0],use_pmm=use_pmm)
    ...
    def forward(self, data):
        ...
        d4 = self.decode4(encoder_feats[3], encoder_feats[2], skip_connection[2])
        d3 = self.decode3(d4, encoder_feats[1], skip_connection[1])
        d2 = self.decode2(d3, encoder_feats[0], skip_connection[0])
