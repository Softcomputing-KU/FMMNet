...
class QaTa(Dataset):
    ...
    def __init__(self, csv_path=None, root_path=None, tokenizer=None, mode='train',image_size=[224,224], sample_ratio=1):
        ...
        if mode == 'train' and sample_ratio != 1:
            random.seed(777)
            data_size = len(self.image_list)
            sample_size = int(data_size * sample_ratio)
            random_indices = random.sample(range(data_size), sample_size)
            self.image_list = [self.image_list[i] for i in random_indices]
            self.caption_list = [self.caption_list[i] for i in random_indices]
    ...
    def __getitem__(self, idx):
        ...
        gt = (torch.sum(gt, axis=0)/1.0).int()
        # For MosMedData
