...
from utils.unetr_black_custum import UnetrUpBlock_PMM
from typing import Type, Any, Callable, Union, List, Optional, cast, Tuple
from torch import Tensor
from utils.fuzzyblock import FuzzyBlock
from utils.PMM import PossibilityMinMaxLayer
...
class GuideDecoder(nn.Module):
    ...
    def __init__(self,in_channels, out_channels, spatial_size, text_len, use_pmm=False) -> None:

        super().__init__()

        self.guide_layer = GuideDecoderLayer(in_channels,text_len)   # for skip
        self.spatial_size = spatial_size
        self.decoder = UnetrUpBlock(2,in_channels,out_channels,3,2,norm_name='BATCH') if use_pmm == False else UnetrUpBlock_PMM(2,in_channels,out_channels,3,2,norm_name='BATCH')

        self.pmm1 = PossibilityMinMaxLayer(out_channels, out_channels)
        self.pmm2 = PossibilityMinMaxLayer(out_channels, out_channels)

        self.use_pmm = use_pmm

    
    def forward(self, vis, skip_vis, txt, skip_encoder):
        ...
        if self.use_pmm == True:
            skip_vis = self.pmm1(skip_vis)
            skip_encoder = self.pmm2(skip_encoder)
            output = self.decoder(vis,skip_vis,skip_encoder)
        else:
...
class Bridger(nn.Module):
    def __init__(self,
        ...
        self.fusion_v = Interactor(d_model=d_model, nhead=nhead, use_fuzzy=use_fuzzy)
        self.fusion_t = Interactor(d_model=d_model, nhead=nhead, use_fuzzy=use_fuzzy)
...
class Interactor(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=128, dropout=0.1,
                 activation="relu", use_fuzzy=False):
        super().__init__()
        ...
        self.linear4 = nn.Linear(int(d_model * 1.5), d_model)
        self.dropout4 = nn.Dropout(dropout)
        self.norm4 = nn.LayerNorm(d_model)
        self.activation2 = _get_activation_fn(activation)   

        self.fuzzy = FuzzyBlock(d_model, d_model//2)
    ...
    def forward(self, tgt, memory,
        ...
        # fuzzy block
        if self.use_fuzzy == True:
            tgt = self.fuzzy(tgt)
            tgt = self.dropout4(self.activation2(self.linear4(tgt)))
            tgt = self.norm4(tgt)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, use_pmm=False):
        super(Decoder, self).__init__()
        self.up = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=2)
        self.conv_bn_relu = nn.Sequential(nn.Conv2d(2*out_channels if use_pmm==False else 3*out_channels, out_channels, kernel_size=3, padding=1), 
...
class FusedDecoder(nn.Module):
    def __init__(self, in_channels, out_channels, d_img = 512, d_txt = 768, d_model = 64, 
                    nhead = 8, num_stages = 3, strides = 2, num_layers = 12, use_fuzzy = False):
        super().__init__()
        self.d_img = d_img
        self.d_txt = d_txt
        self.d_model = d_model
